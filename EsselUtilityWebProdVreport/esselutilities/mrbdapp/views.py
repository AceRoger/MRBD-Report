...
from mrbdapp.serializers import (LoginSerializer, ConsumerDetailsSerializer, MeterImageSerializer, MeterReadingSerializer, UserProfileSerializer)
from mrbdapp.helper import UserProfileToJSON, JobCardToJSON, paginate
from adminapp.models import UserProfile
from dispatch.models import JobCard
...
from .tasks import aysnc_update_jc_status, aysnc_update_rd_jc_status, aysnc_create_image_for_upload, aysnc_create_image_for_unbilled, get_image_file
    ...
    if serializer.is_valid():
        ...
        if user is not None:
            ...
            if (user_profile.status == 'ACTIVE'):
                ...
                devicedetail_obj = DeviceDetail.objects.filter(user_id=user.userprofile.id, imei_no=serializer.validated_data['imei_no'])
...
def get_job_cards(request):
    try:
        ...
        jobcards = JobCard.objects.filter(meterreader= user,record_status='ALLOCATED',is_active=True,is_deleted=False,is_deleted_for_mr=False)
...
def get_deassigned_reassigned_job_cards(request):
    ...
    jobcards = JobCard.objects.filter((Q(record_status='REASSIGNED')|Q(record_status='DEASSIGNED')),meterreader= user,is_deleted_for_mr= False)
                ...
                if serializer.is_valid():
                    ...
                    if consumerdetails_obj is not None:
                        ...
                        if (suspicious_activity_image is not None and suspicious_activity_image["image"] != ""):
                            imageDataDict[consumerdetails_obj.id]['suspicious_activity_image'] = json.dumps(suspicious_activity_image)
                    ...
                    dict['serializer_error_cids'].append("Serializer error : "+str(d.get('consumer_no'))+ " for " +str(serializer.errors))
...
def upload_meter_reading(request):
    ...
    print "Meter Reading upload started"
            ...
            if (meter_image is not None and meter_image["image"] != ""):
                ...
                if serializer.is_valid():
                    ...
                    if meterrading_obj is not None:
                        ...
                        if (suspicious_activity_image is not None and suspicious_activity_image["image"] != ""):
                            imageDataDict[meterrading_obj.id]['suspicious_activity_image'] = json.dumps(suspicious_activity_image)
...
def update_user_profile(request):
    ...
    if serializer.is_valid():
        ...
        if profile_image_obj is not None:
            profile_image_obj.delete()
        return Response({'result':'failure','message':'Serializer Error','responsedata':dict}, status=status.HTTP_400_BAD_REQUEST)

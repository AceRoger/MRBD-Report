...
from dispatch.models import MeterStatus, RouteAssignment, JobCard, ReaderStatus, MeterReading, ValidatorAssignment, \
    UnbilledConsumerAssignment, UnbilledConsumerAssignmentCount, ValidatorAssignmentCount
from scheduleapp.models import BillSchedule, PN33Download, BillScheduleDetails
from adminapp.models import State, City, Utility, EmployeeType, UserProfile, UserPrivilege, UserRole, BillCycle, \
    RouteDetail, UPLD_MTR_RDNG, Availability
...
def validate_jobcard_list(request):
    data = {}
    finallist = []
    objlist = {}
    validationCard1 = []
    ...
    monthYears = []
    billcycleslistpaginate=[]
    currentmonth = None
    page_range=''
    month1 = request.GET.get('month')
    month = request.GET.get('month')
    print "page _number---------------------------", month
    page_number = request.GET.get('page_number')
    print "page _number---------------------------", page_number
    ...
    try:
        ...
        paginator = Paginator(billcycles, 4)  # Show 25 contacts per page
        print " =====================paginator==============================",paginator
        page = request.GET.get('page')
        print "#############dddddd..............request.GET.get('page', 1)................dddddddd#####", page
        page_number = request.GET.get('page_number')
        try:
            billcycleslistpaginate = paginator.page(page_number)
        except PageNotAnInteger:  # If page is not an integer, deliver first page.
            billcycleslistpaginate = paginator.page(1)
        except EmptyPage:  # If page is out of range (e.g. 9999), deliver last page of results.
            billcycleslistpaginate = paginator.page(paginator.num_pages)

        print " *******________________________________________________******billcycleslistpaginate_-------------------",billcycleslistpaginate
        index = billcycleslistpaginate.number - 1  # edited to something easier without index
        max_index = len(paginator.page_range)
        start_index = index - 3 if index >= 3 else 0
        end_index = index + 3 if index <= max_index - 3 else max_index
        page_range = paginator.page_range[start_index:end_index]

        for billcycle in billcycleslistpaginate:
            print " *********************billcycle_-------------------",billcycle
            ...
            if billschedule:
                try:
                    billScheduledetail = BillScheduleDetails.objects.get(billSchedule=billschedule, month=currentmonth,
                                                                         last_confirmed=True)
                    ...
                    for route in routes:
                        ...
                        for routeassignedOne in routeassigned:
                            jobCards = JobCard.objects.filter(routeassigned=routeassignedOne, is_reading_completed=True,
                                                              record_status='COMPLETED', reading_month=currentmonth,
                                                              is_deleted=False, is_active=True)
                            readingreceived = readingreceived + len(jobCards)
                            # query for completed meter readings
                            meterReading = MeterReading.objects.filter(jobcard__routeassigned=routeassignedOne,
                                                                       reading_status='complete',
                                                                       reading_month=currentmonth, is_deleted=False,
                                                                       is_duplicate=False, is_active=True).count()
                            meterReadingCompleted = meterReadingCompleted + int(meterReading)

                            meterReadingV1 = MeterReading.objects.filter(jobcard__routeassigned=routeassignedOne,
                                                                         reading_status='validation1',
                                                                         reading_month=currentmonth, is_deleted=False,
                                                                         is_duplicate=False, is_active=True).count()
                            meterReadingCompletedV1 = meterReadingCompletedV1 + int(meterReadingV1)

                            meterReadingV2 = MeterReading.objects.filter(jobcard__routeassigned=routeassignedOne,
                                                                         reading_status='validation2',
                                                                         reading_month=currentmonth, is_deleted=False,
                                                                         is_duplicate=False, is_active=True).count()
                            meterReadingCompletedV2 = meterReadingCompletedV2 + int(meterReadingV2)

                            meterReadingD = MeterReading.objects.filter(jobcard__routeassigned=routeassignedOne,
                                                                        reading_month=currentmonth, is_duplicate=True,
                    ...
                    validatorAssignment = ValidatorAssignment.objects.filter(user=request.user.userprofile,
                                                                             meterreading__jobcard__routeassigned__routedetail__billcycle=billcycle)
                    ...
                    unBilledConsumers = UnBilledConsumers.objects.filter(reading_month=currentmonth, is_confirmed=False,
                                                                         is_descarded=False,
                                                                         bill_cycle_code=billcycle.bill_cycle_code)
                    ...
                    print 'Exception',e
                    pass
                # paginator = Paginator(finallist, 10)  # Show 25 contacts per page
                # page = request.GET.get('page')
                # print "#############dddddd..............request.GET.get('page', 1)................dddddddd#####", page
                # page_number = request.GET.get('page_number')
                # try:
                #     validationCard1 = paginator.page(page_number)
                # except PageNotAnInteger:  # If page is not an integer, deliver first page.
                #     validationCard1 = paginator.page(1)
                # except EmptyPage:  # If page is out of range (e.g. 9999), deliver last page of results.
                #     validationCard1 = paginator.page(paginator.num_pages)
                # index = validationCard1.number - 1  # edited to something easier without index
                # max_index = len(paginator.page_range)
                # start_index = index - 3 if index >= 3 else 0
                # end_index = index + 3 if index <= max_index - 3 else max_index
                # page_range = paginator.page_range[start_index:end_index]
                ...
                # data = {'finallists': validationCard1, 'overAllConsumers': overAllConsumers,
        #         'overAllReadingsReceived': overAllReadingsReceived, 'overAllValidated': overAllValidated,
        #         'overAllValidatedV1': overAllValidatedV1, 'overAllValidatedV2': overAllValidatedV2,
        #         'overAllPending': overAllPending, 'monthYears': monthYears, 'currentmonth': currentmonth,
        #          'monthinwords': monthh}
        ...
        print e
    if month1 == None:
        print "hiii page not number"
        overAllPending = overAllReadingsReceived - overAllValidated
        data = {'finallists1': billcycleslistpaginate, 'loggedinuser': request.user.userprofile,'finallists':finallist,'page_range':page_range, 'overAllConsumers': overAllConsumers,
                'overAllReadingsReceived': overAllReadingsReceived, 'overAllValidated': overAllValidated,
                'overAllValidatedV1': overAllValidatedV1, 'overAllValidatedV2': overAllValidatedV2,
                'overAllPending': overAllPending, 'monthYears': monthYears, 'currentmonth': currentmonth,
                'monthinwords': monthh}
        print "--------finallists1--------",data

        return render(request, 'validationapp/validation1.html', data)
    else:
        print "hiii page number"
        data = {'finallists1': billcycleslistpaginate,'loggedinuser': request.user.userprofile,'finallists':finallist,'page_range':page_range,'monthYears': monthYears, 'currentmonth': currentmonth,
                'monthinwords': monthh}
        print "--------finallists1--------", data

        data = render(request, 'validationapp/validation1Body.html', data)
...
def validation_level_one(request, billcycle_id=None, month=None, validatorassigned_id=None):
    ...
    if validatorassigned_id:
        ...
        validatorAssigned = ValidatorAssignment.objects.filter((Q(meterreading__reading_status='validation1') | Q(
            meterreading__reading_status='validation2') | Q(meterreading__reading_status='complete')),
                                                               user=request.user, sent_to_revisit=False,
                                                               reading_month=month,
                                                               meterreading__jobcard__routeassigned__routedetail__billcycle_id=billcycle_id,
                                                               meterreading__is_deleted=False,
                                                               meterreading__is_active=True,
                                                               meterreading__is_duplicate=False)
        if validatorAssigned:
            ...
            try:
                ...
                validatorAssignedToShow = ValidatorAssignment.objects.get((Q(
                    meterreading__reading_status='validation1') | Q(meterreading__reading_status='validation2') | Q(
                    meterreading__reading_status='complete')), id=validatorassigned_id, user=request.user,
                                                                          sent_to_revisit=False, reading_month=month,
                                                                          meterreading__jobcard__routeassigned__routedetail__billcycle_id=billcycle_id,
                                                                          meterreading__is_deleted=False,
                                                                          meterreading__is_active=True,
                                                                          meterreading__is_duplicate=False)
                ...
                pass
            return render(request, 'validationapp/validationlevel1.html',
                          {'totalReadingsAssigned': totalReadingsAssigned, 'totalPending': totalPending,
                           'totalValidated': totalValidated, 'billcycle': billcycle, 'route_id': route_id,
                           'data': formData, 'assigned': True, 'prevValidateRecord': prevValidateRecord,
                           'nextValidateRecord': nextValidateRecord, 'reader_status_values': reader_status_values,
                           'meter_status_values': meter_status_values, 'month': month})
        else:
            return render(request, 'validationapp/validationlevel1.html',
                          {'assigned': False, 'month': month, 'billcycle': billcycle})
    else:
...
def validation_level_one_validate(request):
    ...
    if request.POST.get('validatorassignedid'):
        try:
            validatorAssigned = ValidatorAssignment.objects.get(user=request.user, sent_to_revisit=False,
                                                                is_validated=False,
                                                                id=request.POST.get('validatorassignedid'))

            meterReading = MeterReading.objects.get(id=validatorAssigned.meterreading_id)

            meterReadingDuplicate = MeterReading.objects.filter(
                jobcard__consumerdetail=meterReading.jobcard.consumerdetail, reading_month=meterReading.reading_month,
                is_active=False, is_deleted=False, is_duplicate=True)
            if meterReadingDuplicate:
                ...
                messages.add_message(request, messages.INFO,
                                     'Cant validate reading, found duplicate, Added reading in the duplicate bucket!')
                return HttpResponseRedirect(reverse('validate:validation_level_one',
                                                    kwargs={'billcycle_id': billcycleidtopass, 'month': monthpass}))

            meterReadingDuplicate1 = 0
            meterReadingDuplicate1 = MeterReading.objects.filter(
                jobcard__consumerdetail=meterReading.jobcard.consumerdetail, reading_month=meterReading.reading_month,
                is_active=True, is_deleted=False, is_duplicate=False)
            ...
            meterReadingDuplicate2 = UPLD_MTR_RDNG.objects.filter(BILL_MONTH=meterReading.reading_month,
                                                                  CUSTOMER_ID=meterReading.jobcard.consumerdetail.consumer_no)
            ...
            if meterReadingDuplicate2:
                ...
                meterReading.meter_status_v1 = (
                MeterStatus.objects.get(meter_status=request.POST.get('meter_status_v1')))
                meterReading.reader_status_v1 = (
                ReaderStatus.objects.get(reader_status=request.POST.get('reader_status_v1')))
    ...
    if request.POST.get('validatorassignedid'):
        try:
            ...
            if meterReadingDuplicate2:
                ...
                meterReading.meter_status_v2 = (
                MeterStatus.objects.get(meter_status=request.POST.get('meter_status_v1')))
                meterReading.reader_status_v2 = (
                ReaderStatus.objects.get(reader_status=request.POST.get('reader_status_v1')))
...
def validation_level_two(request, billcycle_id=None, month=None, validatorassigned_id=None):
    ...
    if validatorassigned_id:
        ...
        validatorAssigned = ValidatorAssignment.objects.filter(
            (Q(meterreading__reading_status='validation2') | Q(meterreading__reading_status='complete')),
            user=request.user, sent_to_revisit=False, reading_month=month,
            meterreading__jobcard__routeassigned__routedetail__billcycle_id=billcycle_id,
            meterreading__is_deleted=False, meterreading__is_active=True, meterreading__is_duplicate=False)
        if validatorAssigned:
            totalReadingsAssigned = len(validatorAssigned)
            i = 0
            found = False
            ...
            try:
                ...
                validatorAssignedToShow = ValidatorAssignment.objects.get(
                    (Q(meterreading__reading_status='validation2') | Q(meterreading__reading_status='complete')),
                    id=validatorassigned_id, user=request.user, sent_to_revisit=False, reading_month=month,
                    meterreading__jobcard__routeassigned__routedetail__billcycle_id=billcycle_id,
                    meterreading__is_deleted=False, meterreading__is_active=True, meterreading__is_duplicate=False)
                formData = {
                    'consumer': validatorAssignedToShow.meterreading.jobcard.consumerdetail,
                ...
                try:
                    validatorAssignedDummy = ValidatorAssignment.objects.get(assigned_to='validator1',
                                                                             meterreading=validatorAssignedToShow.meterreading,
                                                                             is_validated=True, sent_to_revisit=False,
                                                                             meterreading__is_deleted=False,
                                                                             meterreading__is_active=True,
                                                                             meterreading__is_duplicate=False)
                except ValidatorAssignment.DoesNotExist:
                ...
                pass
            return render(request, 'validationapp/validationlevel2.html',
                          {'totalReadingsAssigned': totalReadingsAssigned, 'totalPending': totalPending,
                           'totalValidated': totalValidated, 'billcycle': billcycle, 'route_id': route_id,
                           'data': formData, 'assigned': True, 'prevValidateRecord': prevValidateRecord,
                           'nextValidateRecord': nextValidateRecord,
                           'previousValidatorRemark': validatorAssignedDummy.remark if validatorAssignedDummy else '',
                           'reader_status_values': reader_status_values, 'meter_status_values': meter_status_values,
                           'month': month})
        else:
            return render(request, 'validationapp/validationlevel2.html',
                          {'assigned': False, 'month': month, 'billcycle': billcycle})
        ...
        if validatorAssigned:
            ...
            if found == False:
                ...
                try:
                    validatorAssignedDummy = ValidatorAssignment.objects.get(assigned_to='validator1',
                                                                             meterreading=validatorAssigned[
                                                                                 0].meterreading, is_validated=True,
                                                                             sent_to_revisit=False,
                                                                             meterreading__is_deleted=False,
                                                                             meterreading__is_active=True,
                                                                             meterreading__is_duplicate=False)
                except ValidatorAssignment.DoesNotExist:
    ...
    if request.POST.get('validatorassignedid'):
        try:
            ...
            if meterReadingDuplicate2:
                ...
                meterReading.meter_status_v2 = (
                MeterStatus.objects.get(meter_status=request.POST.get('meter_status_v2')))
                meterReading.reader_status_v2 = (
                ReaderStatus.objects.get(reader_status=request.POST.get('reader_status_v2')))
                ...
                if request.is_ajax():
                    ...
                    messages.add_message(request, messages.INFO, 'Consumer validated successfully!')
                    return HttpResponseRedirect(reverse('validate:validation_level_two',
                                                        kwargs={'billcycle_id': billcycleidtopass, 'month': monthpass}))
        except (ValidatorAssignment.DoesNotExist, MeterReading.DoesNotExist), e:
            print e
            print "An unexpected error is occured!"
...
def validation_level_one_revisit(request):
    ...
    if request.POST.get('validatorassignedid'):
        ...
        return HttpResponseRedirect(
            reverse('validate:validation_level_one', kwargs={'billcycle_id': billcycleidtopass, 'month': monthpass}))
...
def validation_level_two_revisit(request):
    ...
    if request.POST.get('validatorassignedid'):
        ...
        messages.add_message(request, messages.ERROR, 'An unexpected error is occured! Try again')
        return HttpResponseRedirect(
            reverse('validate:validation_level_two', kwargs={'billcycle_id': billcycleidtopass, 'month': monthpass}))
...
def validation_level_one_listview(request, billcycle_id=None, month=None):
    ...
    validatorAssigned = ValidatorAssignment.objects.filter((Q(meterreading__reading_status='validation1') | Q(
        meterreading__reading_status='validation2') | Q(meterreading__reading_status='complete')),
                                                           user=request.user.userprofile, sent_to_revisit=False,
                                                           reading_month=month,
                                                           meterreading__jobcard__routeassigned__routedetail__billcycle_id=billcycle_id,
                                                           meterreading__is_deleted=False, meterreading__is_active=True,
                                                           meterreading__is_duplicate=False)
    if validatorAssigned:
        totalReadingsAssigned = len(validatorAssigned)
        for validatorAssignedOne in validatorAssigned:
        ...
        totalPending = totalReadingsAssigned - totalValidated
        return render(request, 'validationapp/validation_level_one_listview.html',
                      {'totalReadingsAssigned': totalReadingsAssigned, 'totalPending': totalPending,
                       'totalValidated': totalValidated, 'alldata': allData, 'assigned': True, 'billcycle': billcycle,
                       'month': month})
    else:
...
def validation_level_one_list_export(request, billcycle_id=None, month=None):
    try:
        ...
        writer.writerow(
            ['RouteID', 'Consumer Number', 'Meter Number', 'Consumer Name', 'Meter Reader Name', 'Current Reading',
             'Status'])
        ...
        if validatorAssigned:
            ...
            for validatorAssignedOne in validatorAssigned:
                ...
                tempList.append(
                    str("'") + str(validatorAssignedOne.meterreading.jobcard.consumerdetail.consumer_no).encode(
                        'utf-8'))
                ...
                tempList.append((
                                validatorAssignedOne.meterreading.jobcard.meterreader.first_name + ' ' + validatorAssignedOne.meterreading.jobcard.meterreader.last_name).encode(
                    'utf-8'))
...
def validation_level_two_listview(request, billcycle_id=None, month=None):
    ...
    validatorAssigned = ValidatorAssignment.objects.filter(
        (Q(meterreading__reading_status='validation2') | Q(meterreading__reading_status='complete')),
        user=request.user.userprofile, sent_to_revisit=False, reading_month=month,
        meterreading__jobcard__routeassigned__routedetail__billcycle_id=billcycle_id, meterreading__is_deleted=False,
        meterreading__is_active=True, meterreading__is_duplicate=False)
    if validatorAssigned:
        totalReadingsAssigned = len(validatorAssigned)
        for validatorAssignedOne in validatorAssigned:
            if validatorAssignedOne.is_validated == True:
...
def unbillconsumer(request, billcycle_id=None, month=None):
    ...
    billCycle = BillCycle.objects.get(id=billcycle_id)
    try:
        unBilledConsumersTotal = UnBilledConsumers.objects.filter(reading_month=month, is_confirmed=False,
                                                                  bill_cycle_code=billCycle.bill_cycle_code)
        totalunBilledConsumers = len(unBilledConsumersTotal)
    except Exception, e:
        print e
    try:
        unbilledConsumerAssignment = UnbilledConsumerAssignment.objects.filter(user_id=request.user.userprofile.id,
                                                                               reading_month=month, is_confirmed=False,
                                                                               unbillconsumer__bill_cycle_code=billCycle.bill_cycle_code,
                                                                               unbillconsumer__is_descarded=False,
                                                                               unbillconsumer__is_confirmed=False)
        ...
        pass
    return render(request, 'validationapp/unbilllconsumer.html',
                  {'data': unBilledConsumers, 'totalunBilledConsumers': totalunBilledConsumers, 'billCycle': billCycle,
                   'month': month, 'monthtoshow': monthhh(month)})
    ...
    try:
        ...
        if unbilledConsumerAssignment:
            for dataOne in unbilledConsumerAssignment:
                ...
                tempList.append(
                    dataOne.unbillconsumer.meterreader.first_name + ' ' + dataOne.unbillconsumer.meterreader.last_name)
...
def unbillconsumerreview(request, billcycle_id=None, month=None, cons_id=None):
    ...
    try:
        ...
        print e
        pass
    return render(request, 'validationapp/unbilllconsumerreview.html',
                  {'consumer': unbillconsumerToShow, 'prevValidateRecord': prevValidateRecord,
                   'nextValidateRecord': nextValidateRecord, 'billcycle_id': billcycle_id, 'month': month,
                   'monthtoshow': monthhh(month), 'reader_status_values': reader_status_values,
...
def verifyconsumer(request):
    ...
    try:
        consumerDetails = ConsumerDetails.objects.get(consumer_no=unBilledConsumer.consumer_no,
                                                      bill_cycle__id=request.POST.get('billcyclecode'),
                                                      bill_month=request.POST.get('month'))
        data = {'success': 'true'}
...
def add_unbilled_consumer(request):
    ...
    try:
        unbilledConsumerAssignment = UnbilledConsumerAssignment.objects.get(user_id=request.user.userprofile.id,
                                                                            reading_month=request.POST.get('billmonth'),
                                                                            unbillconsumer=unBilledConsumer)
    except UnbilledConsumerAssignment.DoesNotExist:
        print "found exception 2"
    try:
        unbilledConsumerAssignmentCount = UnbilledConsumerAssignmentCount.objects.get(
    ...
    routeDetail = RouteDetail.objects.filter(route_code=request.POST.get('route_id'),
                                             bill_month=request.POST.get('billmonth'))
    ...
    try:
        consumerDetails = ConsumerDetails.objects.get(consumer_no=request.POST.get('consumer_no'),
                                                      bill_month=request.POST.get('billmonth'))
        ...
        jobCard1 = JobCard.objects.filter(consumerdetail=consumerDetails, reading_month=request.POST.get('billmonth'),is_active=True,meterreader__isnull=False).last()
        if (jobCard1):
            try:
                ...
                messages.add_message(request, messages.INFO,
                                     'Cant add reading, reading already exist! Added to Duplicate reading bucket!')
                data = {'success': 'true', 'add': 6}  # added as duplicate reading successfully
                return HttpResponse(json.dumps(data), content_type='application/json')

            except MeterReading.DoesNotExist:
                try:
                    routeAssignment = RouteAssignment.objects.get(routedetail__route_code=request.POST.get('route_id'),
                                                                  reading_month=request.POST.get('billmonth'),
                                                                  routedetail__billcycle=billCycle)
                    jobCard = JobCard(
        ...
        unBilledConsumer.save()
        messages.add_message(request, messages.INFO,
                             'Cant add Reading! Consumer does not exist! Reading cannot be added. Please regularise the consumer.')
...
def dupicatelist(request, billcycle_id=None, month=None):
    ...
    meterReading = MeterReading.objects.filter(jobcard__routeassigned__routedetail__billcycle_id=billcycle_id,
                                               reading_month=month, is_duplicate=True, is_active=False,
                                               is_deleted=False)

    return render(request, 'validationapp/duplicate.html',
                  {'data': meterReading, 'billCycle': billCycle, 'month': month, 'monthtoshow': monthhh(month)})
...
def dupicatelist_export(request, billcycle_id=None, month=None):
    try:
        ...
        for meterReadingOne in meterReading:
            ...
            tempList.append(
                meterReadingOne.jobcard.meterreader.first_name + ' ' + meterReadingOne.jobcard.meterreader.last_name)
...
def viewduplicate(request, billcycle_id, month, duplicate_id):
    ...
    return render(request, 'validationapp/viewduplicate.html',
                  {'data': formData, 'billCycle': billCycle, 'month': month, 'monthtoshow': monthhh(month),
                   'reader_status_values': reader_status_values, 'meter_status_values': meter_status_values,
                   'route_id': route_id})
...
def validate_consumer_search(request):
    print 'validate_consumer_search|request in with',request.POST
    print 'validate_consumer_search|request in with',request.user
    print 'validate_consumer_search|request in with',request.user.id
    print 'validate_consumer_search|request in with',request.POST.get('validator')
    try:
        validatorAssigned = None
        try:
            if request.POST.get('validator') == '1':
                print 'validatorAssigned =======>1'
                validatorAssigned = ValidatorAssignment.objects.get((Q(meterreading__reading_status='validation1') | Q(
                    meterreading__reading_status='validation2') | Q(meterreading__reading_status='complete')),
                                                                    user=request.user.id, sent_to_revisit=False,
                                                                    reading_month=request.POST.get('searchmonth'),
                                                                    meterreading__jobcard__consumerdetail__consumer_no=request.POST.get(
                                                                        'searchconsumer'),
                                                                    meterreading__is_deleted=False,
                                                                    meterreading__is_active=True,
                                                                    meterreading__is_duplicate=False)
                print 'validatorAssigned==>',validatorAssigned
            else:
                validatorAssigned = ValidatorAssignment.objects.get(
                    (Q(meterreading__reading_status='validation2') | Q(meterreading__reading_status='complete')),
                    user=request.user, sent_to_revisit=False, reading_month=request.POST.get('searchmonth'),
                    meterreading__jobcard__consumerdetail__consumer_no=request.POST.get('searchconsumer'),
                    meterreading__is_deleted=False, meterreading__is_active=True, meterreading__is_duplicate=False)
            data = {'success': 'true', 'validator': request.POST.get('validator'),
                    'month': request.POST.get('searchmonth'),
                    'billcycleid': validatorAssigned.meterreading.jobcard.routeassigned.routedetail.billcycle_id,
                    'validatorAssigned':validatorAssigned.id
                    }  # true
            return HttpResponse(json.dumps(data), content_type='application/json')
        except ValidatorAssignment.DoesNotExist:
            print 'validatorAssigned==> user doesnot exist'
...
def validator_summery(request, month=None, billcycle_id=None, role=None):
    ...
    for billcycle in billcycles:
        try:
            ...
            if billschedule:
                try:
                    billScheduledetail = BillScheduleDetails.objects.get(billSchedule=billschedule, month=month1,
                                                                         last_confirmed=True)
    ...
    try:
        ...
        if role_to_pass == 0:
            meterReadersAsValidator1 = UserProfile.objects.filter((Q(type='VALIDATOR_1') | Q(type='VALIDATOR_2')),
        ...
        for meterReadersAsValidator1One in meterReadersAsValidator1:
            ...
            if billcycle_id_to_pass == 0:
                for billcycle in billcyclesArray:
                    ...
                    totalAssigned = ValidatorAssignment.objects.filter(user=meterReadersAsValidator1One,
                                                                       sent_to_revisit=False, reading_month=month1,
                                                                       meterreading__is_deleted=False,
                                                                       meterreading__is_active=True,
                                                                       meterreading__is_duplicate=False,
                                                                       meterreading__jobcard__is_active=True,
                                                                       meterreading__jobcard__is_deleted=False,
                                                                       meterreading__jobcard__is_deleted_for_mr=False,
                                                                       meterreading__jobcard__routeassigned__routedetail__billcycle_id=billcycle.id).count()

                    totalAssignedPending = ValidatorAssignment.objects.filter(user=meterReadersAsValidator1One,
                                                                              sent_to_revisit=False,
                                                                              reading_month=month1,
                                                                              meterreading__is_deleted=False,
                                                                              meterreading__is_active=True,
                                                                              meterreading__is_duplicate=False,
                                                                              meterreading__jobcard__is_active=True,
                                                                              meterreading__jobcard__is_deleted=False,
                                                                              meterreading__jobcard__is_deleted_for_mr=False,
                                                                              meterreading__jobcard__routeassigned__routedetail__billcycle_id=billcycle.id,
                                                                              meterreading__reading_status=e).count()
                ...
                totalAssigned = ValidatorAssignment.objects.filter(user=meterReadersAsValidator1One,
                                                                   ...
                                                                   meterreading__jobcard__is_deleted=False,
                                                                   meterreading__jobcard__is_deleted_for_mr=False,
                                                                   meterreading__jobcard__routeassigned__routedetail__billcycle_id=billcycle_id_to_pass).count()
                ...
                totalAssignedPending = ValidatorAssignment.objects.filter(user=meterReadersAsValidator1One,
                                                                          ...
                                                                          meterreading__jobcard__is_deleted=False,
                                                                          meterreading__jobcard__is_deleted_for_mr=False,
                                                                          meterreading__jobcard__routeassigned__routedetail__billcycle_id=billcycle_id_to_pass,
                                                                          meterreading__reading_status=e).count()
        ...
        print 'exception ', str(traceback.print_exc())
    return render(request, 'validationapp/validate_summery1.html',
                  {'monthYears': monthYears, 'currentmonth': month1, 'validators': validators,
                   'billcyclesArray': billcyclesArray, 'billcycle_id_to_pass': billcycle_id_to_pass,
                   'role_to_pass': str(role_to_pass)})
...
def validator_summery_export(request, month=None, billcycle_id=None, role=None):
        ...
        for meterReadersAsValidator1One in meterReadersAsValidator1:
            ...
            if billcycle_id_to_pass == 0:
                for billcycle in billcyclesArray:
                    if meterReadersAsValidator1One.type == "VALIDATOR_1":
                        role = 'Validator 1'
                        totalAssignedAll = ValidatorAssignment.objects.filter((Q(
                            meterreading__reading_status='validation1') | Q(
                            meterreading__reading_status='validation2') | Q(meterreading__reading_status='complete')),
                                                                              user=meterReadersAsValidator1One,
                                                                              sent_to_revisit=False,
                                                                              reading_month=month1,
                                                                              meterreading__is_deleted=False,
                                                                              meterreading__is_active=True,
                ...
                if meterReadersAsValidator1One.type == "VALIDATOR_1":
                    ...
                    totalAssignedAll = ValidatorAssignment.objects.filter((Q(
                        meterreading__reading_status='validation1') | Q(meterreading__reading_status='validation2') | Q(
                        meterreading__reading_status='complete')), user=meterReadersAsValidator1One,
                                                                          sent_to_revisit=False, reading_month=month1,
                                                                          meterreading__is_deleted=False,
                                                                          meterreading__is_active=True,
                                                                          meterreading__is_duplicate=False,
                    ...
                    role = 'Validator 2'
                    totalAssignedAll = ValidatorAssignment.objects.filter(
                        (Q(meterreading__reading_status='validation2') | Q(meterreading__reading_status='complete')),
                        user=meterReadersAsValidator1One, sent_to_revisit=False, reading_month=month1,
                        meterreading__is_deleted=False, meterreading__is_active=True, meterreading__is_duplicate=False,
                        meterreading__jobcard__routeassigned__routedetail__billcycle_id=billcycle_id_to_pass,
                        meterreading__jobcard__is_active=True, meterreading__jobcard__is_deleted=False,
                        meterreading__jobcard__is_deleted_for_mr=False)
    ...
    try:
        ...
        for validator in validators:
            ...
            if reading_status:
                tempList.append(
                    reading_status.current_meter_reading if reading_status.current_meter_reading else '----')
                tempList.append(
                    reading_status.current_meter_reading_v1 if reading_status.current_meter_reading_v1 else '----')
                tempList.append(
                    reading_status.current_meter_reading_v2 if reading_status.current_meter_reading_v2 else '----')
            else:
                tempList.append('----')
                tempList.append('----')
                tempList.append('----')
            ...
            if reading_status:
                try:
                    tempList.append('----' if (
                    reading_status.comment is None or reading_status.comment == '') else reading_status.comment)
                except:
                    tempList.append('----')
                try:
                    tempList.append('----' if (
                    reading_status.comment_v1 is None or reading_status.comment_v1 == '') else reading_status.comment_v1)
                except:
                    tempList.append('----')
                try:
                    tempList.append('----' if (
                    reading_status.comment_v2 is None or reading_status.comment_v2 == '') else reading_status.comment_v2)
                except:
                    tempList.append('----')
                try:
                    tempList.append(reading_status.image_remark_v1 if (
                    reading_status.image_remark_v1 is not None or reading_status.image_remark_v1 is not '') else '----')
                except:
                    tempList.append('----')
                try:
                    tempList.append(reading_status.image_remark_v2 if (
                    reading_status.image_remark_v2 is not None or reading_status.image_remark_v2 is not '') else '----')
                except:
                    tempList.append('----')
            ...
            if reading_status:
                if reading_status.updated_by_v1:
                    tempList.append(
                        reading_status.updated_by_v1.first_name + ' ' + reading_status.updated_by_v1.last_name)
                else:
                    tempList.append('---')

                if reading_status.updated_by_v2:
                    tempList.append(
                        reading_status.updated_by_v2.first_name + ' ' + reading_status.updated_by_v2.last_name)
                else:
                    tempList.append('---')
            else:
                tempList.append('---')
...
def check_duplicate_existence(request):
    try:
        meterReading = MeterReading.objects.get(id=request.POST.get('meterreadingid'))
        consumer = meterReading.jobcard.consumerdetail
        month = meterReading.jobcard.reading_month
        try:
            MeterReading.objects.get(jobcard__consumerdetail=consumer, reading_month=month, is_duplicate=False,reading_status__in=['validation2','complete'],
                                     is_active=True, is_deleted=False)
            data={'success':'true','result':'exist'}
        except MeterReading.DoesNotExist, e:
            data={'success':'true','result':'notexist'}
        except MeterReading.MultipleObjectsReturned, e:
            data={'success':'true','result':'exist'}
    except Exception,e:

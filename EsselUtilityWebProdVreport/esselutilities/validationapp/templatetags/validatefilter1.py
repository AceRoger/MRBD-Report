...
def getFisrtImage(consumer):
    try:
        jobcard = JobCard.objects.get(consumerdetail__consumer_no = consumer.consumer_no, is_active = True, is_deleted = False, reading_month = consumer.month)
        meterReading = MeterReading.objects.get(jobcard = jobcard)
...
def getSecondImage(consumer):
    ...
    try:
        jobcard = JobCard.objects.get(consumerdetail__consumer_no = consumer.consumer_no, is_active = True, is_deleted = False, reading_month = monthToShow)
        meterReading = MeterReading.objects.get(jobcard = jobcard)
        return meterReading.image_url
    except:
        return None
